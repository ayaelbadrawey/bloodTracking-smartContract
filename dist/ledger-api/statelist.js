...
const queryPaginatedResponse_1 = require("../utils/queryPaginatedResponse");
const state_1 = require("./state");
const logger = fabric_shim_1.newLogger('STATELIST');
...
class StateList {
    ...
    use(...stateClasses) {
        for (const stateClass of stateClasses) {
            if (!(stateClass.prototype instanceof state_1.State)) {
                throw new Error(`Cannot use ${stateClass.name} as type State`);
            }
    ...
    async getHistory(key) {
        ...
        while (value) {
            ...
            const state = state_1.State.deserialize(value.getValue().toBuffer(), this.supportedClasses);
            const historicState = new state_1.IHistoricState(value.getTimestamp().getSeconds().toInt(), value.getTxId(), state);
    ...
    async getAssetsByRange(startKey, endKey) {
        const ledgerStartKey = this.ctx.stub.createCompositeKey(this.name, state_1.State.splitKey(startKey));
        const ledgerEndKey = this.ctx.stub.createCompositeKey(this.name, state_1.State.splitKey(endKey));
     ...
     */
    async queryWithPagination(queryString, pageSize, bookmark) {
        /*
       ...
       */
        ...
        const queryPaginatedRes = new queryPaginatedResponse_1.QueryPaginationResponse(result.metadata.fetched_records_count, result.metadata.bookmark);
